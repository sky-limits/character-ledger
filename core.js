(function(root) {
  ...
  function craftingProgress(recipe) {
    const required = recipe.requirements.reduce((sum,item) => sum + item.required, 0);
    const have = recipe.requirements.reduce((sum,item) => sum + Math.min(item.have,item.required), 0);
  ...
  function validate(input) {
    ...
    for(const recipe of d.crafting){
      try {
        ...
        for(const item of recipe.requirements){
          ...
          num(item.have,'owned amount');
        }
      } catch(error) {throw new Error('Crafting recipe "'+recipe.id+'": '+error.message);}
    }
    ...
    return {format:d.format,version:d.version,systems:d.systems,characters:d.characters,art:d.art,adjustments:d.adjustments,rewards:d.rewards,redemptions:d.redemptions,crafting:d.crafting};
  }
  root.LedgerCore={totalXP,progress,rewardRows,craftingProgress,validate,safeURL,copy};
