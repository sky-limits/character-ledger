...
assert.equal(versions.length,6,'Expected six cache-keyed browser assets in v1.01.');
assert.ok(versions.every(version=>version==='1.01'),'Every browser asset must use the v1.01 cache key.');
...
assert.match(html,/enhancements\.css\?v=1\.01/);
assert.match(html,/enhancements\.js\?v=1\.01/);
assert.match(html,/data-lightbox-nav="-1"/);
assert.match(html,/data-lightbox-nav="1"/);

const app=fs.readFileSync(path.join(root,'app.js'),'utf8');
const css=fs.readFileSync(path.join(root,'style.css'),'utf8');
const enhancements=fs.readFileSync(path.join(root,'enhancements.js'),'utf8');
const enhancementCss=fs.readFileSync(path.join(root,'enhancements.css'),'utf8');
...
assert.match(enhancements,/global-ledger-search/);
assert.match(enhancements,/normalizeFocalPoint/);
assert.match(enhancements,/points-desc/);
assert.match(enhancements,/ArrowLeft/);
assert.match(enhancementCss,/prefers-reduced-motion:reduce/);
assert.match(enhancementCss,/forced-colors:active/);
